...
class App(QMainWindow):
    ...
    def initUI(self):
        ...
        # Create latex or mathml
        self.latexRadio = QRadioButton('LaTeX', self)
        self.mathmlRadio = QRadioButton('MathML', self)
        self.latexRadio.setChecked(True)  # Set MathML as the default
        #self.latexRadio.clicked.connect(self.updateOutputFormat)
        #self.mathmlRadio.clicked.connect(self.updateOutputFormat)
        ...
        buttons.addWidget(self.latexRadio)
        buttons.addWidget(self.mathmlRadio)
    ...
    def displayPrediction(self, prediction=None):
        selected_format = 'latex' if self.latexRadio.isChecked() else 'mathml'
        if self.isProcessing:
            pageSource = """<center>
            ...
            if prediction is not None:
                if selected_format == 'mathml':
                    mathml_prediction = latex2mathml.converter.convert(prediction)
                    clipboard.copy(mathml_prediction)
                    self.textbox.setText("{equation}".format(equation=mathml_prediction))
                else:
